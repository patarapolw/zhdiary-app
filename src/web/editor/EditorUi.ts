...
import { fetchJSON, quizDataToContent, dotGetter, fixData, DateFormat } from "../util";
import swal from "sweetalert";
import toastr from "toastr";
import flatpickr from "flatpickr";
import DatetimeNullable from "../components/DatetimeNullable";
...
interface IColumn {
    ...
    label?: string;
    width: number;
    type?: "markdown" | "datetime" | "tag";
}

@Component({
    components: {DatetimeNullable, EntryEditor},
    template: h(".stretched#editor-ui", [
        h(".editor-control", [
            h(".editor-nav", [
                h("button.btn", {attrs: {
                    "v-on:click": "offset = 0"
            ...
            h("thead", [
                h("tr", [
                    ...
                    h("th", {attrs: {
                        ...
                        }}, "{{ c.label || (c.name[0].toLocaleUpperCase() + c.name.substr(1)) }}"),
                        h("span", {attrs: {
                            "v-if": "sortBy === c.name"
                        }}, "{{ desc ? ' ▲' : ' ▼'}}")
                    ]),
            ...
            h("tbody", [
                ...
                h("tr", {attrs: {
                    ...
                    h("td", {attrs: {
                        ...
                        h(".wrapper", [
                            h(".wrapped", {attrs: {
                                "v-if": "a[2].type === 'markdown'",
                                "v-html": "getHtml(d, a[0])"
                            }}),
                            h(".wrapped", {attrs: {
...
export default class EditorUi extends Vue {
    ...
    get tableCols() {
        const cols: IColumn[] = [
            {name: "key", width: 200},
            {name: "front", width: 400, type: "markdown"},
            {name: "back", width: 400, type: "markdown"},
            {name: "mnemonic", width: 400, type: "markdown"},
            {name: "tag", width: 200, type: "tag"},
            {name: "srsLevel", label: "SRS Level", width: 200},
            {name: "nextReview", label: "Next Review", width: 250, type: "datetime"},
            {name: "created", width: 250, type: "datetime"},
            {name: "modified", width: 250, type: "datetime"}
        ];
    ...
    get tableWidth(): number {
        return this.colWidths.checkbox
        + this.tableCols.map((c) => c.width).reduce((a, v) => a + v);
    }
